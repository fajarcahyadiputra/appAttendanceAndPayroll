			...
			<div class="card-body">
				...
				<div class="table-responsive">
					<table class="table table-bordered table-striped table-hover" id="tableAdmin" style="width: 100%">
						...
						<tbody>
							<?php $no=1; foreach ($admin as $key => $dt): 
							?>
							<tr>
								<td><?php echo $no++ ?></td>
...
<script>
	$(document).ready(function(){
		...
		$('#tableAdmin').on('click', '#tombol_hapus', function(){
		...
		$('#tableAdmin').on('click','#tombol_edit', function(){
			const id = $(this).data('id');
			$.ajax({
				url: base_url+'Admin/tampilEditAdmin',
				data:{"id":id},
				dataType: 'json',
				type: 'post',
				success: function(hasil){
				   ...
				   <div class="form-group">
						...
						<input required="" type="hidden" name="id_user" class="form-control" value="${hasil.id_user}">
					</div>
					<div class="form-group">
						<label>Nama</label>
					...
					<div class="form-group">
						...
						<select require name="level" id="level" class="form-control">
							...
							<option ${hasil.level === 'super_admin'?'selected':'' } value="super_admin">super_admin</option>
						</select>
					</div>
					<div class="form-group">
						<label>Jenis Kelamin</label>
					...
					<div class="form-group">
						<label>Satatus Aktif</label>
						<select require name="status_aktif" id="status_aktif" class="form-control">
							<option ${hasil.level === 'ya'?'selected':'' } value="ya">Ya</option>
							<option ${hasil.level === 'no'?'selected':'' } value="no">No</option>
						</select>
					</div>
				</div>
				<div class="modal-footer">
		...
		//end
			...
			//ajax tampil data pengguna
			$('#tableAdmin').on('click','#tombol_info', function(){
			const id = $(this).data('id');
			$.ajax({
				url: base_url+'admin/tampilEditAdmin',
				data:{"id":id},
				...
				type: 'post',
				success: function(hasil){
				$('#form_info').html(`<div class="modal-body">
				   <div class="form-group">
						<label>Username</label>
						<input readonly class="form-control" value="${hasil.username}">
					</div>
					<div class="form-group">
						<label>Nama</label>
						<input readonly class="form-control" value="${hasil.nama}">
					</div>
					<div class="form-group">
						<label>No HP</label>
						<input readonly class="form-control" value="${hasil.no_hp}">
					</div>
					<div class="form-group">
						<label>Level</label>
						<input readonly class="form-control" value="${hasil.level}">
					</div>
					<div class="form-group">
						<label>Jenis Kelamin</label>
						<input readonly class="form-control" value="${hasil.jenis_kelamin}">
					</div>
					<div class="form-group">
						<label>Satatus Aktif</label>
						<input readonly class="form-control" value="${hasil.status_aktif}">
					</div>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
		...
		$('#tableAdmin').on('click', '#tombol_password', function(){
			const id = $(this).data('id');
